...
a <- read.table('card1_8_potato.bed',header=F)

ab <- read.table('card1_aln_region.xls',header=F)
...
for (i in 1:spe_num){
    ...
    left <- min(a1[,3])
    right <- max(a1[,4])
    segments(left - inter,(spe_num+1-i)*10,right + inter,(spe_num+1-i)*10,lwd=3.5,lend=0, col=rgb(128,128,128,max=255))
}
