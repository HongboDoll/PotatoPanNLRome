...
ls ../52_potato_genomes/*fa|awk -F '/' '{print $NF}'|awk -F '.' '{print $1}' | while read i
do
    name=`echo $i | awk -F '.' '{print $1}'`
    nlr_num=`cat nlr_gff3/${i}*gff3 | awk '$3=="mRNA"' | wc -l`
...
#	awk '$4!=2' clustered_results/${name}.clustered.txt > t && mv t clustered_results/${name}.clustered.txt
#	python3 x.flt_eight_genes_insertion.py integrated_gff3/${name}.integratedNLR.gff3 clustered_results/${name}.clustered.txt t && mv t clustered_results/${name}.clustered.txt
