...
i2 = int(sys.argv[1])  # left/right flanking length, i2 
...
for k in chr_name_revise:
	if len(chr_name_revise[k]) <= 1:
		if chr_name_revise[k][0][0] > i2:
			chr_name_flanking[k] = [[chr_name_revise[k][0][0]-i2, chr_name_revise[k][0][1]+i2, chr_name_revise[k][0][2], chr_name_revise[k][0][3]]]
		else:
			chr_name_flanking[k] = [[chr_name_revise[k][0][0], chr_name_revise[k][0][1]+i2, chr_name_revise[k][0][2], chr_name_revise[k][0][3]]]
	else:
		for n in range(0, len(chr_name_revise[k])):
			...
			if n == 0:
				...
				if left2 - right1 <= i2*2 and orient1 == orient2:
					flank = int(math.floor((left2 - right1)/2))
					if chr_name_revise[k][n][0] > i2:
						chr_name_flanking[k].append([chr_name_revise[k][n][0]-i2, chr_name_revise[k][n][1]+flank, chr_name_revise[k][n][2], chr_name_revise[k][n][3]])
					else:
						chr_name_flanking[k].append([chr_name_revise[k][n][0], chr_name_revise[k][n][1]+flank, chr_name_revise[k][n][2], chr_name_revise[k][n][3]])
				else:
					if chr_name_revise[k][n][0] > i2:
						chr_name_flanking[k].append([chr_name_revise[k][n][0]-i2, chr_name_revise[k][n][1]+i2, chr_name_revise[k][n][2], chr_name_revise[k][n][3]])
					else:
						chr_name_flanking[k].append([chr_name_revise[k][n][0], chr_name_revise[k][n][1]+i2, chr_name_revise[k][n][2], chr_name_revise[k][n][3]])
				...
				if orient0 == orient1 and orient1 == orient2:
					if left1 - right0 > i2*2:
						flank_left = i2
					elif left1 - right0 <= i2*2:
						flank_left = int(math.floor((left1 - right0)/2))
					if left2 - right1 > i2*2:
						flank_right = i2
					elif left2 - right1 <= i2*2:
