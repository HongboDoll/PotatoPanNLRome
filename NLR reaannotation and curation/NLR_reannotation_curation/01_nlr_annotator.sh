...
do
    ...
    java -jar $nlr_path/NLR-Annotator.jar -i ${i}.nlr.xml -o ${i}.nlr.txt  -f $i ${i}.nlr.flanking2k.fa 2000 -m ${i}.nlr.motif.bed -g ${i}.NLR.gff3 -b ${i}.nlr.bed
done

rm -rf NLR_gff/; mkdir NLR_gff/
cp *.NLR.gff3 NLR_gff/

rm -rf NLR_2k_gff NLR_2k_seq/; mkdir NLR_2k_gff NLR_2k_seq/
ls NLR_gff/*gff3 | while read i
do
    name=`echo $i | awk -F '/' '{print $2}' | awk -F '.' '{print $1}'`
    grep -v '#' $i | sort -k1,1 -k4,4n -k5,5n | ./remove_nested_NLR_revise_flanking.py 2000 > NLR_2k_gff/${name}.NLR.2k.gff3
    cat NLR_2k_gff/${name}.NLR.2k.gff3 | while read n
    do
        seq=`echo $n | awk -F '=' '{print $2}'`
        chr=`echo $n | awk '{print $1}'`
        start=`echo $n | awk '{print $4}'`
        end=`echo $n | awk '{print $5}'`
        samtools faidx ${name}_hifiasm.purged*.fa ${chr}:${start}-${end} | awk '{if($1~/>/){print ">""'"$seq"'"}else{print $0}}' >> NLR_2k_seq/${name}.NLR.2k.fa
    done
done
